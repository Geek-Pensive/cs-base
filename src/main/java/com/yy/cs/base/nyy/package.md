...
这种机制需要依赖公司UDB的相关验证接口，UDB已有相关支持。目前在一些重要的业务过程中已经成功使用了这种机制，欢迎联系咨询。
...
在一些安全性要求很高的场合，也可以对data进行加密，这样既除了保证数据的完整性，还可以加强数据的保密性。加密算法使用3DES。
...
| 消息中appId的值 | （事先分配给对应业务的id）                                                                                                      |
| 消息中sign的值  | （为空即可）                                                                                                                    |
| 消息中data的值  | `BASE64(  3DES({"chId":"Zfb","payer":"小王"}, ljfadjaf023ur32lj)  )`<br/>注意：在HTTP请求中，生成的字节码需要转换成BASE64，**不需要**进行URL Encode |
...
这种机制需要依赖公司UDB的相关验证接口，UDB已有相关支持。
...
| 消息中data的值  | `BASE64(  3DES({"chId":"Zfb","payer":"小王"}, 58wfwY5w33o5w95we)  )`<br/>注意：在HTTP请求中，生成的字节码需要转换成BASE64，**不需要**进行URL Encode |
