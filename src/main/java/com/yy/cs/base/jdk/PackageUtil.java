...
public class PackageUtil {
     ...
     * 
     * @param basePackage 包
     * @param listChildren 是否遍历子包
     * @return
     */
    public static List<Class<?>> listClassByPackage(String basePackage, boolean listChildren, ClassFilter filter) {
        List<String> classNames = getClassName(basePackage, listChildren);
        List<Class<?>> classes = new LinkedList<>();
        for (String className : classNames) {
            try {
                Class<?> clz = Class.forName(className);
                if (null == filter || filter.filter(clz)) {
                    classes.add(clz);
                }
            } catch (Exception e) {
                log.error("parse ClassString[" + className + "] to Class error ", e);
            }
        }
        return classes;
    }
